...
from browser import HSTEP, VSTEP, SCROLL_STEP, print_tree
from browserskia_render import Text, Element
from browser import tree_to_list, INHERITED_PROPERTIES, INPUT_WIDTH_PX
from browser_skia import get_font, linespace
from task_scheduler import Task
from animator import diff_styles, absolute_bounds_for_obj, NumericAnimation, map_translation, parse_transform
from animator import DrawText, DrawLine, DrawRRect, paint_visual_effects
from animator import Blend, Transform, DrawOutline
from browserskia_render import parse_outline, style, \
    paint_outline, dpx, cascade_priority, \
    is_focusable, get_tabindex, CSSParser, mainloop, Chrome
from browser_tabs import URL, HTMLParser, DrawImage, \
...
class ProtectedField:
    ...
    def read(self, notify):
        ...
        if tools.PRINT_INVALIDATION_DEPENDENCIES:
            prefix = ""
...
@tools.patch(absolute_bounds_for_obj)
...
@tools.patch(paint_visual_effects)
...
class AccessibilityNode:
    def compute_bounds(self):
        ...
        # ``children`` can be either a ProtectedField or a plain list.  In the
        # ProtectedField case we have to call ``get()`` to read the value,
        # otherwise we can iterate over the list directly.  See Chapter 15 of
        # the tutorial for details.  Always avoid calling ``get()`` on a list.
        child_container = inline.layout_object.children
        if isinstance(child_container, ProtectedField):
            child_lines = child_container.get()
        else:
            child_lines = child_container
...
# =========================
# Lab16 UI fixes (scroll wheel, clickable back button, smaller tab text/icon)
# =========================

# 1) Make tab rectangles wider (so the title isn't cramped)
def _patched_tab_rect(self, i):
    x0 = self.newtab_rect.right() + self.padding + i * 260
    return skia.Rect.MakeLTRB(x0, 2, x0 + 260, 28)

try:
    Chrome.tab_rect = _patched_tab_rect
except Exception:
    pass

# 2) Smaller tab font + icon (do it inside paint so we don't depend on Chrome.__init__)
_old_chrome_paint = Chrome.paint
def _chrome_paint_smaller(self):
    # Shrink UI typography a bit
    try:
        self.font = get_font(16, "normal", "roman")
        self.font_height = linespace(self.font)
        # Slightly tighter padding looks better with smaller font
        self.padding = 4
    except Exception:
        pass
    return _old_chrome_paint(self)

Chrome.paint = _chrome_paint_smaller

# 3) Ensure clicks work in Lab16 even if Browser doesn't provide handle_click/hover
@tools.patch(Browser)
class Browser:
    def handle_click(self, event):
        # event is SDL_MouseButtonEvent or similar; fall back safely
        try:
        ...
        try:
            ...
            if y < self.chrome.bottom:
                # Back button
                if self.chrome.back_rect.contains(x, y):
                    self.lock.release()
                    self.go_back()
                    return

                # New tab
                if self.chrome.newtab_rect.contains(x, y):
                    self.lock.release()
                    self.new_tab(URL("https://browser.engineering/"))
                    return

                # Address bar focus
                if self.chrome.address_rect.contains(x, y):
                    self.focus_addressbar()
                    self.set_needs_raster()
                    return

                # Tabs (switch / close)
                for i, tab in enumerate(self.tabs):
                    bounds = self.chrome.tab_rect(i)
                    if bounds.contains(x, y):
                        # Close region: last ~28px of the tab
                        if x >= bounds.right() - 28:
                            try:
                                self.close_tab(i)
                            except Exception:
                                pass
            ...
            if not self.active_tab:
                return
            content_x = x
            content_y = y - self.chrome.bottom
            task = Task(self.active_tab.click, content_x, content_y)
            self.active_tab.task_runner.schedule_task(task)
            self.needs_animation_frame = True
        finally:
            # If we returned early above, lock may already be released.
            if self.lock.locked():
                self.lock.release()

    def handle_hover(self, event):
        # Optional: forward hover to accessibility hover logic if present
        try:
            x, y = int(event.x), int(event.y)
        except Exception:
            return
        if y < self.chrome.bottom:
            return
        try:
            self.pending_hover = (x, y - self.chrome.bottom)
            self.set_needs_accessibility()
        except Exception:
            pass

# 4) Override mainloop to add mouse wheel scrolling (trackpad/mouse)
    ...
    while True:
        if sdl2.SDL_PollEvent(ctypes.byref(event)) != 0:
            if event.type == sdl2.SDL_QUIT:
                ...
                # SDL: wheel.y > 0 means scroll up; < 0 means scroll down
                steps = abs(int(event.wheel.y)) if event.wheel.y else 1
                if event.wheel.y > 0:
