...
def handle_connection(conx):
    req = conx.makefile("b")
    reqline = req.readline().decode("utf8", errors="replace")

    # Ignore non-HTTP or binary probes
    if not reqline.startswith(("GET", "POST")):
        conx.close()
        return
    ...
    headers = {}
    while True:
        line = req.readline().decode("utf8", errors="replace")
        if line == "\r\n":
            break
        k, v = line.split(":", 1)
    ...
    if "content-length" in headers:
        ...
        body = req.read(length).decode("utf8", errors="replace")
    else:
        body = None
