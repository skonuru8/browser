...
from browser import INHERITED_PROPERTIES, cascade_priority
from browser import tree_to_list, CSSParser
from browser import Text, Element, URL
from browser_skia import parse_color, parse_blend_mode
from browser_skia import paint_tree, BlockLayout, DocumentLayout, TextLayout, InputLayout
from task_scheduler import MeasureTime, SingleThreadedTaskRunner, TaskRunner
from task_scheduler import Tab, Browser, Task, REFRESH_RATE_SEC, Chrome, JSContext
from task_scheduler import CommitData
...
class Blend(VisualEffect):
    def __init__(self, opacity, blend_mode, node, children):
        ...
        if tools.USE_COMPOSITING and self.should_save:
...
class CompositedLayer:
    ...
    def raster(self):
        ...
        if tools.SHOW_COMPOSITED_LAYER_BORDERS:
...
class Browser:
    ...
    def handle_click(self, e):
        self.lock.acquire(blocking=True)
        # Handle clicks in the chrome (tab bar & address bar) separately
        if e.y < self.chrome.bottom:
            # First see if the click hit a tab close button.  Each tab gets
            # an 'x' drawn on its right edge; clicking in this area will
            # close the tab.  We compute the close area based on the tab
            # rectangle and the font metrics.
            # Width of the close button: character width plus two paddings.
            # Use the multiplication sign (\u00d7) if available, fallback to x.
            try:
                cross_char = "×"
                char_w = self.chrome.font.measureText(cross_char)
            except Exception:
                cross_char = "x"
                char_w = self.chrome.font.measureText(cross_char)
            close_w = char_w + 2 * self.chrome.padding
            for i, tab in enumerate(self.tabs):
                bounds = self.chrome.tab_rect(i)
                # The close rect starts one padding unit before the text area
                close_left = bounds.right() - close_w - self.chrome.padding
                close_right = bounds.right() - self.chrome.padding
                close_top = bounds.top() + self.chrome.padding
                close_bottom = close_top + self.chrome.font_height + self.chrome.padding
                if close_left <= e.x <= close_right and \
                   close_top <= e.y <= close_bottom:
                    self.close_tab(i)
                    self.lock.release()
                    return

            # If not clicking on a close button, delegate to Chrome.click
            self.focus = None
            # Some Chrome implementations define a click handler; call it if present
            try:
                handled = self.chrome.click(e.x, e.y)
            except AttributeError:
                handled = False
            # Switching tabs or navigating back may require a re‑raster
            self.set_needs_raster()
        else:
            # Click occurred in the content area.  Focus the content and
            # forward the click to the active tab.
            if self.focus != "content":
                self.set_needs_raster()
            self.focus = "content"
            # blur the chrome so the address bar loses focus
            try:
                self.chrome.blur()
            except AttributeError:
                pass
    ...
    def close_tab(self, index):
        """Close the tab at the given index and activate a new tab."""
        if not (0 <= index < len(self.tabs)):
            return
        tab = self.tabs[index]
        # Terminate the tab's task runner so it stops processing events
        try:
            tab.task_runner.set_needs_quit()
        except Exception:
            pass
        del self.tabs[index]
        # Choose a new active tab if any remain
        if self.tabs:
        ...
        self.clear_data()
        # We need to redraw the UI after closing a tab
        self.set_needs_raster()
