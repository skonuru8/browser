...
from browser import BLOCK_ELEMENTS, HTMLParser
from browserskia_render import Text, Element
from browser import tree_to_list, INHERITED_PROPERTIES
from browser import INPUT_WIDTH_PX
from browser import COOKIE_JAR, URL
from browser_skia import get_font, linespace
from task_scheduler import MeasureTime, JSContext
from task_scheduler import Task, TaskRunner, SingleThreadedTaskRunner
from animator import diff_styles
...
from animator import PaintCommand, DrawText, \
    DrawLine, DrawRRect
from animator import Blend, Transform
from browserskia_render import paint_outline, \
    dpx, cascade_priority, style, is_focusable, get_tabindex, \
    CSSParser, mainloop, Browser, Chrome, Tab, AccessibilityNode, \
...
BROKEN_IMAGE = skia.Image.open("Not_loaded.png")
...
class Frame:
    ...
    def scrollup(self):
        """Scroll the frame up by one step."""
        # Subtract a step and clamp to the top.  ``clamp_scroll`` will
        # ensure we do not scroll above zero.
        self.scroll = self.clamp_scroll(self.scroll - SCROLL_STEP)
        self.scroll_changed_in_frame = True
...
class Tab:
    def __init__(self, browser, tab_height):
        ...
        # Store a human‑readable title for this tab.  This will be set
        # whenever a page is loaded (see load method).  Defaults to empty.
        self.title = ""
    ...
    def load(self, url, payload=None):
        ...
        # Extract the <title> element from the loaded DOM and store it on
        # the tab.  If no title is found, fall back to the URL's domain.
        try:
            def find_title(node):
                from browserskia_render import Text, Element  # lazy import to avoid cycles
                if isinstance(node, Element) and node.tag.lower() == "title":
                ...
                for child in getattr(node, "children", []):
                    ...
                    if t:
                        return t
                return None
            title = find_title(self.root_frame.nodes)
            if title:
                self.title = title
            else:
                # Use the last component of the URL path or the domain
                try:
                    host = url.host
                except Exception:
                    host = str(url)
            ...
            # On any failure, leave title unchanged
            pass

    def get_js(self, url):
        origin = url.origin()
        if tools.FORCE_CROSS_ORIGIN_IFRAMES:
    ...
    def scrollup(self):
        """Scroll the focused frame (or root frame) up by one step."""
        frame = self.focused_frame or self.root_frame
        frame.scrollup()
        self.needs_accessibility = True
        self.set_needs_paint()
...
class Browser:
    ...
    def handle_up(self):
        """Scroll up within the active tab.  Mirrors ``handle_down``."""
        self.lock.acquire(blocking=True)
        if self.root_frame_focused:
            if not self.active_tab_height:
                self.lock.release()
                return
            # Subtract a scroll step and clamp to zero.  Do not call
            # ``clamp_scroll`` here because it is not defined on Browser
            new_scroll = self.active_tab_scroll - SCROLL_STEP
            if new_scroll < 0:
                new_scroll = 0
            ...
            self.lock.release()
            return
        # Schedule the scroll on the tab so that it runs on the tab thread.
    ...
    def go_back(self):
        ...
        This method delegates to the active tab's ``go_back`` method,
        which manipulates the per-tab history list. After scheduling
        the history change, it clears any pending render state and
        requests a new animation frame so the previous page will be
        drawn when it loads.
        """
        # Do nothing if there is no active tab or no history to go back to
        if not self.active_tab:
            return
        # Schedule the tab-level history navigation on the tab thread
        try:
            task = Task(self.active_tab.go_back)
            self.active_tab.task_runner.schedule_task(task)
        except Exception:
            # In single-threaded mode, just call go_back directly
            try:
                self.active_tab.go_back()
            except Exception:
                return
        # Wipe the current drawn content so the next commit paints fresh
        self.clear_data()
        # Mark that we need a new frame to reflect the navigated page
        self.needs_animation_frame = True
        # Ensure a re-raster and re-draw on the next cycle
        try:
    ...
    def close_tab(self, index):
        """Close the tab at the given index and activate a new tab if possible."""
        # Ensure index is in range
        if not (0 <= index < len(self.tabs)):
            return
        tab = self.tabs[index]
        # Request the tab's task runner to quit so the thread stops processing
        try:
        ...
        del self.tabs[index]
        # Pick a new active tab if any remain
        if self.tabs:
            # Prefer the previous tab; otherwise the first tab
            new_index = index - 1 if index > 0 else 0
            self.set_active_tab(self.tabs[new_index])
        else:
            self.active_tab = None
        # Clear rendering data and schedule a redraw
        self.clear_data()
        self.needs_animation_frame = True
