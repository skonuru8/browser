...
from browser import WIDTH, HEIGHT, HSTEP, VSTEP, SCROLL_STEP, URL
from browser import Text, Element, HTMLParser
from browser import TagSelector, DescendantSelector
from browser import INHERITED_PROPERTIES
from browser import tree_to_list
from browser import INPUT_WIDTH_PX
from browser import URL
from browser_skia import get_font, linespace, paint_tree
from browser_skia import DocumentLayout, LineLayout
from task_scheduler import MeasureTime, SingleThreadedTaskRunner, TaskRunner, Chrome
from task_scheduler import Task
from animator import JSContext, diff_styles
from animator import absolute_bounds_for_obj
from animator import NumericAnimation
from animator import paint_visual_effects
from animator import DrawText, DrawCompositedLayer, DrawOutline, \
    DrawLine, DrawRRect
from animator import Tab, Browser, CSSParser
from animator import BlockLayout, TextLayout, InputLayout
...
class Chrome:
    def paint(self):
        ...
        for i, tab in enumerate(self.browser.tabs):
            ...
            # Determine the title to display: prefer the page title, fallback to
            # the URL string or an index label
            label = getattr(tab, "title", None)
            if not label:
                try:
                    label = str(tab.url)
                except Exception:
            ...
            label = label.strip() if label else f"Tab {i}"
            # Determine an icon: use the first character of the label, uppercased
            icon_char = label[0].upper() if label else 'T'
            # Icon sizing: square with height equal to the font height
            icon_size = self.font_height
            icon_left = bounds.left() + self.padding
            icon_top = bounds.top() + (bounds.height() - icon_size) / 2
            icon_rect = skia.Rect.MakeLTRB(
                icon_left,
                icon_top,
                icon_left + icon_size,
                icon_top + icon_size)
            # Draw a rounded rect as the icon background
            icon_radius = icon_size / 4
            # Use a neutral light gray background for the icon
            cmds.append(DrawRRect(icon_rect, icon_radius, "#e0e0e0"))
            # Draw the icon letter centered inside the icon rect
            icon_char_width = self.font.measureText(icon_char)
            icon_x = icon_left + (icon_size - icon_char_width) / 2
            icon_y = icon_top + (icon_size - self.font_height) / 2 + self.font_height - linespace(self.font)
            cmds.append(DrawText(icon_x, icon_y, icon_char, self.font, color))

            # Compute available width for the label text: from end of icon to
            # beginning of close button minus padding
            # Approximate close button width as width of '×' plus 2*padding
            try:
                close_char_w = self.font.measureText("×")
            except Exception:
                close_char_w = self.font.measureText("x")
            close_w = close_char_w + 2 * self.padding
            text_x_start = icon_rect.right() + self.padding
            text_x_end = bounds.right() - close_w - self.padding
            max_label_width = max(0, text_x_end - text_x_start)
            # Truncate the label if it's too long to fit
            truncated_label = label
            while self.font.measureText(truncated_label) > max_label_width and len(truncated_label) > 1:
                truncated_label = truncated_label[:-1]
            if truncated_label != label:
                # Indicate truncation with an ellipsis
                if len(truncated_label) > 1:
                    truncated_label = truncated_label[:-1] + "…"
            label_y = bounds.top() + self.padding
            cmds.append(DrawText(
                text_x_start,
                label_y,
                truncated_label,
            ...
            # Draw the close button: a light gray box with an × inside
            close_left = bounds.right() - close_w - self.padding
            close_top = bounds.top() + self.padding
            close_rect = skia.Rect.MakeLTRB(
                close_left,
                close_top,
                bounds.right() - self.padding,
                close_top + self.font_height + self.padding)
            corner_radius = (self.font_height + self.padding) / 4
            cmds.append(DrawRRect(close_rect, corner_radius, "#f0f0f0"))
            # Darken the × for contrast
            cross_x = close_left + (close_rect.width() - close_char_w) / 2
            cross_y = close_top + (close_rect.height() - self.font_height) / 2 + self.font_height - linespace(self.font)
            cmds.append(DrawText(cross_x, cross_y, "×", self.font, color))
    ...
    def click(self, x, y):
        """
        Handle clicks on the browser chrome (tab strip, new-tab button,
        back button, and address bar). This method is invoked by
        ``Browser.handle_click()`` if the click occurs above the page
        content. It must return True if it handled the click, allowing
        the caller to skip further processing.

        The default implementation here implements history navigation and
        tab switching. It explicitly resets the browser's focus when
        clicking on chrome elements so that subsequent key events are
        directed to the appropriate place.
        """
        # Always clear the browser focus when interacting with the chrome
        # so keystrokes do not leak into the content area.
        self.browser.focus = None
        # New tab button: open a new tab at the browser's home page
        if self.newtab_rect.contains(x, y):
            try:
                # Use the URL class imported from browser for compatibility
                self.browser.new_tab(URL("https://browser.engineering/"))
            except Exception:
                # Fallback if new_tab isn't implemented: do nothing
                pass
            return True
        # Back button: navigate to the previous page in this tab's history
        if self.back_rect.contains(x, y):
            try:
                # Defer to the browser's history management
                self.browser.go_back()
            except Exception:
                pass
            return True
        # Address bar: enter into address-editing mode
        if self.address_rect.contains(x, y):
            self.focus = "address bar"
            self.address_bar = ""
            return True
        # Tab strip: activate the clicked tab (but not the close area)
        for i, tab in enumerate(self.browser.tabs):
            if self.tab_rect(i).contains(x, y):
                try:
                    self.browser.set_active_tab(tab)
                except Exception:
                    # As a fallback, update active_tab directly so the
                    # renderer will draw the new tab content on the next frame.
...
def mainloop(browser):
    ...
    while True:
        if sdl2.SDL_PollEvent(ctypes.byref(event)) != 0:
            if event.type == sdl2.SDL_QUIT:
                ...
                if ctrl_down:
                    ...
                    # Scroll up when the up arrow key is pressed.  Without
                    # this branch the browser only scrolls downward.
                    browser.handle_up()
                elif event.key.keysym.sym == sdl2.SDLK_TAB:
                    browser.handle_tab()
        ...
        if not tools.USE_BROWSER_THREAD:
